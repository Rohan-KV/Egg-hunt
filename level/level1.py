...
def run(window, clock):
    pygame.display.set_caption("Duck's Adventure - Level 1")
    ...
    grass_tiles = [
        (x, y) for y, row in enumerate(m1) for x, col in enumerate(row) if col == "0"
    ]
    ...
    egg_positions = random.sample(available_tiles, 10)
    ...
    while True:
        ...
        if is_walkable(m1, new_x, new_y, TILE_SIZE):
            duck_x = new_x
            duck_y = new_y
        ...
        eggs_text = f"Eggs: {collected_eggs}/10"
