...
def run(window, clock):
    pygame.display.set_caption("Duck's Adventure - Level 2")
    ...
    grass_tiles = [
        (x, y) for y, row in enumerate(m2) for x, col in enumerate(row) if col == "0"
    ]
    ...
    snake_tiles = random.sample(snake_candidates, 5)
    ...
    while True:
        ...
        if is_walkable(m2, new_x, new_y, TILE_SIZE):
            duck_x = new_x
            duck_y = new_y
        ...
        eggs_text = f"Eggs: {collected_eggs}/15"
